 ...
 * Lookup keys in the hashmap
 * 
 * @param hashmap   [N] uint32 tensor containing the hashmap (key-value pairs)
 * @param keys      [M] uint32 tensor containing the keys to be looked up
 * @return          [M] uint32 tensor containing the values of the keys
 */
...
torch::Tensor hashmap_lookup_3d_cuda(
    ...
    // Allocate output tensor
    auto output = torch::empty({coords.size(0)}, torch::dtype(torch::kUInt32).device(hashmap.device()));

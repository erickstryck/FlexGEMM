...
from . import config
from .indice_conv_fwd_masked_implicit_gemm import indice_conv_fwd_masked_implicit_gemm_kernel
...
@triton.jit
def indice_conv_fwd_masked_implicit_gemm_splitk_kernel(
    input,
    weight,
    bias,
    neighbor,
...
def indice_conv_fwd_masked_implicit_gemm_splitk_configs(input, weight, bias, neighbor, sorted_idx, valid_kernel, valid_kernel_seg):
...
def indice_conv_fwd_masked_implicit_gemm_splitk_keys(input, weight, bias, neighbor, sorted_idx, valid_kernel, valid_kernel_seg):
...
@autotune(
    config_fn=indice_conv_fwd_masked_implicit_gemm_splitk_configs,
    key_fn=indice_conv_fwd_masked_implicit_gemm_splitk_keys,
)
def indice_conv_fwd_masked_implicit_gemm_splitk(
    input: torch.Tensor,
    weight: torch.Tensor,
    bias: torch.Tensor,
    ...
    if SPLITK == 1:
        ...
        indice_conv_fwd_masked_implicit_gemm_splitk_kernel[grid](
            input, weight, bias, neighbor, sorted_idx, output,
            M, LOGN, LOGM, Ci, Co, V,
            valid_kernel=valid_kernel,
            valid_kernel_seg=valid_kernel_seg,
